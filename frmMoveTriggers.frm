VERSION 5.00
Begin VB.Form frmMoveTriggers 
   Caption         =   "Copy Triggers"
   ClientHeight    =   6015
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   7530
   Icon            =   "frmMoveTriggers.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   6015
   ScaleWidth      =   7530
   ...
   End
End
Attribute VB_Name = "frmMoveTriggers"
...
If flag% = 0 Then
   MsgBox "No Triggers have been selected."
   Exit Sub
End If

sysbut% = MsgBox("Are you sure you want to copy the selected triggers?", 4, "Copy Confirm")
If sysbut% <> vbYes Then Exit Sub

frmMoveTriggers.MousePointer = 11
Set conn = New adodb.Connection
Set cmd1 = New adodb.Command
...
Next
conn.Close
frmMoveTriggers.MousePointer = 1
...
Loop
oconn.Close
If CorA$ = "A" Then strText = Replace(strText, "CREATE TRIGGER", "ALTER TRIGGER")
Sql = strText
cmd1.CommandText = Sql
cmd1.Execute
On Error GoTo 0
